...
class SintelFramePair4ThatVideo(Dataset):
    def __init__(self, root_dir, downsizeFactorList=[1,1/2.,1/4.,1/8.,1/16.], 
        ...
        for sampleFile in os.listdir(self.current_set_dir): # self.sorted_dir
            if sampleFile.endswith((".png","_files")) or sampleFile.endswith((".jpg","_files")):
                tmpformat = sampleFile[-4:]
                set_path += [path.join(self.current_set_dir, 'frame_'+format(frameID,'04')+tmpformat)]
                frameID+=1
        set_path.sort()
    ...
    def __getitem__(self, idx):
        ...
        imgName2 = path.join(curpath, 'frame_'+format(frameID+frameStride,'04d')+tmpformat)
