...
def train_model(model, dataloaders, dataset_sizes, 
                loss_1_to_2, loss_2_to_1, loss_groupSparsity, loss_filterSmoothness,
                optimizer, scheduler, 
            ...
            for sample in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase=='train'):
                    if phase=='train':  # backward + optimize only if in training phase
                        ...
                        lossRec1to2 = loss_1_to_2(img1, img2, embFeature1_to_2, phase)
                        lossRec2to1 = loss_2_to_1(img2, img1, embFeature2_to_1, phase)
                        loss += lossRec1to2
                        loss += lossRec2to1
                        running_loss_reconstruction += lossRec1to2.item()*img1.size(0)
                        running_loss_reconstruction += lossRec2to1.item()*img1.size(0)
                        
                        lossSmooth2to1 = loss_filterSmoothness(embFeature2_to_1)
                        lossSmooth1to2 = loss_filterSmoothness(embFeature1_to_2)
                        loss += lossSmooth2to1
                        loss += lossSmooth1to2
                        running_loss_filterSmoothness += lossSmooth2to1.item()*img1.size(0)
                        running_loss_filterSmoothness += lossSmooth1to2.item()*img1.size(0)
                        ...
                        running_loss_groupSparsity += loss_groupSparse2to1.item()*img1.size(0)
                        running_loss_groupSparsity += loss_groupSparse1to2.item()*img1.size(0)
                        
                        
                        #loss += loss_1_to_2(img1, img2, embFeature1_to_2, phase)
                        #loss += loss_2_to_1(img2, img1, embFeature2_to_1, phase)
                        #loss += loss_filterSmoothness(embFeature2_to_1)
                        #loss += loss_filterSmoothness(embFeature1_to_2)
                        #loss += loss_groupSparsity(embFeature2_to_1)*1
                        #loss += loss_groupSparsity(embFeature1_to_2)*1
                        loss.backward()
                        optimizer.step()
                    else: 
                        model.eval()                        
                        ...
                        #target = torch.zeros_like(embFeature2_to_1).to(device)
                        #loss += loss_groupSparsity(embFeature2_to_1)*1
                        #loss += loss_groupSparsity(embFeature1_to_2)*1
                        #loss += loss_filterSmoothness(embFeature2_to_1)
                        #loss += loss_filterSmoothness(embFeature1_to_2)
                ...
                del loss
                if iterCount%100==0:
                    print('\t{}/{} loss: {:.6f} LossRec:{:.4f}, LossSmooth:{:.4f}, LossSparse:{:.4f}'.
                          format(
                              iterCount, 
                    ...
                    fn = open(log_filename,'a')        
                    fn.write('\t{}/{} loss: {:.6f} LossRec:{:.4f}, LossSmooth:{:.4f}, LossSparse:{:.4f}\n'.
                             format(
                                 iterCount, 
                                 len(dataloaders[phase]), 
                                 print2screen_avgLoss, print2screen_avgLoss_Rec, 
