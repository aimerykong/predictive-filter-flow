...
def train_model(model, dataloaders, dataset_sizes, 
                ...
                warpImgWithScale8, warpImgWithScale16, warpImgWithScale32,
    ...
    for epoch in range(num_epochs):        
        ...
        for phase in ['train', 'val']:
            ...
            for sample in dataloaders[phase]:
                ...
                imgListA1,imgListB1, imgListA2,imgListB2, imgListA4,imgListB4 = sample[:6]
                imgListA8,imgListB8, imgListA16,imgListB16,imgListA32,imgListB32 = sample[6:]              
                ...
                imgListA1 = imgListA1.to(device)
                imgListB1 = imgListB1.to(device)
                N = imgListA32.size(0)
                ...
                with torch.set_grad_enabled(phase=='train'):
                    ...
                    recImgA32x32 = warpImgWithScale32(imgListB1, PFFx32_2to1)
                    recImgA32x32 = recImgA32x32.detach()
                    recImgB32x32 = warpImgWithScale32(imgListA1, PFFx32_1to2)
                    recImgB32x32 = recImgB32x32.detach()
                    ...
                    recImgA16x16 = warpImgWithScale16(recImgA32x32, PFFx16_2to1)                 
                    recImgA16x16 = recImgA16x16.detach()                     
                    recImgB16x16 = warpImgWithScale16(recImgB32x32, PFFx16_1to2)                 
                    recImgB16x16 = recImgB16x16.detach() 
                    ...
                    recImgA8x8 = warpImgWithScale8(recImgA16x16, PFFx8_2to1)                 
                    recImgA8x8 = recImgA8x8.detach()                     
                    recImgB8x8 = warpImgWithScale8(recImgB16x16, PFFx8_1to2)                 
                    recImgB8x8 = recImgB8x8.detach()  
                    ...
                    recImgA4x4 = warpImgWithScale4(recImgA8x8, PFFx4_2to1)                 
                    recImgA4x4 = recImgA4x4.detach()
                    recImgB4x4 = warpImgWithScale4(recImgB8x8, PFFx4_1to2)                 
                    recImgB4x4 = recImgB4x4.detach()
                    ...
                    PFFx2_2to1, _ = model(imgListA2, recImgA4x2)
                    _, PFFx2_1to2 = model(recImgB4x2, imgListB2)
                    
                    lossRecA2 = loss_pixelReconstruction(recImgA4x2, imgListA2, PFFx2_2to1)
                    loss2s += lossRecA2
                    lossRecB2 = loss_pixelReconstruction(recImgB4x2, imgListB2, PFFx2_1to2)
                    loss2s += lossRecB2
                    lossFlow4ReconA = loss_warp4reconstruction(recImgA4x2, imgListA2, PFFx2_2to1) 
                    loss2s += lossFlow4ReconA
                    lossFlow4ReconB = loss_warp4reconstruction(recImgB4x2, imgListB2, PFFx2_1to2)
                    loss2s += lossFlow4ReconB
                    lossSmooth2to1 = loss_filterSmoothness(PFFx2_2to1)
                    lossSmooth1to2 = loss_filterSmoothness(PFFx2_1to2)
                    loss2s += lossSmooth2to1
                    loss2s += lossSmooth1to2
                    
                    if phase=='train': 
                        loss2s.backward()   
                    
                    recImgA2x2 = warpImgWithScale2(recImgA4x4, PFFx2_2to1)                 
                    recImgA2x2 = recImgA2x2.detach()
                    recImgB2x2 = warpImgWithScale2(recImgB4x4, PFFx2_1to2)                 
                    recImgB2x2 = recImgB2x2.detach()
                    ...
                    #recImgA2x2 = recImgA2x2.to(supplDevice)
                    #imgListA1 = imgListA1.to(supplDevice)
                    #recImgB2x2 = recImgB2x2.to(supplDevice)
                    #imgListB1 = imgListB1.to(supplDevice)                    
                    #loss_filterSmoothness.maxRangePixel += 1
                    ...
                    NCHW = imgListA1.size() 
                    th, tw = 256, 256
                    x1 = random.randint(0, NCHW[3] - tw)
                    y1 = random.randint(0, NCHW[2] - th)
                    recImgA2x2 = recImgA2x2[:,:,y1:y1+th,x1:x1+tw]
                    imgListA1 = imgListA1[:,:,y1:y1+th,x1:x1+tw]
                    recImgB2x2 = recImgB2x2[:,:,y1:y1+th,x1:x1+tw]
                    imgListB1 = imgListB1[:,:,y1:y1+th,x1:x1+tw]
                    
                    #####  at the scale of current interest  ######
                    _, PFFx1A_1to2 = model(recImgA2x2, imgListA1) 
                    lossRecA = loss_pixelReconstruction(recImgA2x2, imgListA1, PFFx1A_1to2)
                    loss1s += lossRecA
                    running_loss_reconstruction += lossRecA.item()*N                    
                    
                    lossFlow4ReconA = loss_warp4reconstruction(recImgA2x2, imgListA1, PFFx1A_1to2)
                    reconsturctedImageA = loss_warp4reconstruction.reconstructImage
                    loss1s += lossFlow4ReconA
                    running_loss_flow4warpRecon += lossFlow4ReconA.item()*N
                    
                    #lossSmooth2to1A = loss_filterSmoothness(PFFx2A_2to1)
                    lossSmooth1to2A = loss_filterSmoothness(PFFx1A_1to2)
                    #loss += lossSmooth2to1A
                    loss1s += lossSmooth1to2A
                    #running_loss_filterSmoothness += lossSmooth2to1A.item()*N
                    running_loss_filterSmoothness += lossSmooth1to2A.item()*N                         
                                       
                    loss_imageGradientA = loss_imageGradient(reconsturctedImageA, imgListA1)*N
                    loss1s += loss_imageGradientA                    
                    running_loss_imageGradient += loss_imageGradientA.item()*N
                    
                    
                    
                    
                    _, PFFx1B_1to2 = model(recImgB2x2, imgListB1) 
                    lossRecB = loss_pixelReconstruction(recImgB2x2, imgListB1, PFFx1B_1to2)
                    loss1s += lossRecB
                    running_loss_reconstruction += lossRecB.item()*N
                    
                    lossFlow4ReconB = loss_warp4reconstruction(recImgB2x2, imgListB1, PFFx1B_1to2)
                    reconsturctedImageB = loss_warp4reconstruction.reconstructImage
                    loss1s += lossFlow4ReconB
                    running_loss_flow4warpRecon += lossFlow4ReconB.item()*N
                    
                    lossSmooth1to2B = loss_filterSmoothness(PFFx1B_1to2)
                    loss1s += lossSmooth1to2B
                    running_loss_filterSmoothness += lossSmooth1to2B.item()*N                          
                                       
                    loss_imageGradientB = loss_imageGradient(reconsturctedImageB, imgListB1)*N
                    loss1s += loss_imageGradientB                    
